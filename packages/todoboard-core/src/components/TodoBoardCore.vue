...
import TodoBoard from "./todo-board.vue";
import EditTaskModal from "./edit-task-modal.vue";
...
// Provide state via composables
const tasksStore = provideTasks(props.hooks);
const uiStore = provideUI();
...
// Watch props for data changes and push into internal state
watch(
	() => props.initialTasks,
	(newTasks) => {
		tasksStore.setTasksInternal(newTasks);
	},
	{ immediate: true }
);
...
watch(
	() => props.initialConfig,
	(newConfig) => {
		tasksStore.setBoardConfigInternal(newConfig);
	},
...
watch(
	() => props.initialFilter,
	(newFilter) => {
		tasksStore.setFilterInternal(newFilter);
	},
	{ immediate: true, deep: true }
);
